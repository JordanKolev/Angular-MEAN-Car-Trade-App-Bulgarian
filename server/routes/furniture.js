...
function validateFurnitureForm (payload) {
  ...
  if (!payload || typeof payload.make !== 'string' || payload.make.length < 0) {
    isFormValid = false
    errors.make = 'Make must be more than 1 symbols.'
  }

  if (!payload || typeof payload.model !== 'string' || payload.model.length < 0) {
    isFormValid = false
    errors.model = 'Model must be more than 1 symbols.'
  }
  ...
  if (!payload || typeof payload.description !== 'string' || payload.description.length < 50) {
    isFormValid = false
